...
Tools allow the LLM to perform actions in your codebase. OpenCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools), [MCP servers](/docs/mcp-servers), or [UTCP providers](/docs/utcp-providers).
...
UTCP (Universal Tool Calling Protocol) providers offer a simpler way to add external tools without maintaining persistent connections. UTCP tools are queried on-demand and executed via HTTP requests.

[Learn more](/docs/utcp-providers) about configuring UTCP providers.
