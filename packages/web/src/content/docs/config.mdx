...
You can configure UTCP (Universal Tool Calling Protocol) providers you want to use through the `utcp` option.
...
UTCP is a simpler protocol than MCP for adding external tools. Unlike MCP which requires a persistent connection, UTCP providers are queried on-demand.

[Learn more here](/docs/utcp-providers).
